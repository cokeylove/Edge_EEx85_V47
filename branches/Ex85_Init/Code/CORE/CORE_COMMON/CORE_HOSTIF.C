...
void KBC_DataPending(BYTE nPending)
{
    ...
    KBDataPending[KBPendingRXCount] = nPending;
    KBPendingRXCount++;
    SetServiceSendFlag();
}
...
{
    BYTE buffer_data;
    buffer_data=KBDataPending[KBPendingTXCount];
    KBPendingTXCount++;
...
void service_send(void)
{
   ...
   SetServiceSendFlag();
    ...
    if ( PM1PendingRXCount > 0 ) {
      if(IS_MASK_SET(PM1STS,OBF)) return;
        Data_To_PM1(GetPM1_PendingData());
        return;
    }
    //if ( PM2PendingRXCount > 0 ) {
    //  if(IS_MASK_SET(PM2STS,OBF)) return;
...
static BYTE translate_to_pc(BYTE data_word, BYTE break_prefix_flag)
{
    ...
    {
        0x00, 0x43, 0x41, 0x3F, 0x3D, 0x3B, 0x3C, 0x58, // 00 ~ 07 0x00~0x07
        0x64, 0x44, 0x42, 0x40, 0x3E, 0x0F, 0x29, 0x59, // 08 ~ 15
        0x65, 0x38, 0x2A, 0x70, 0x1D, 0x10, 0x02, 0x5A, // 16 ~ 23 0x10~0x17
        0x66, 0x71, 0x2C, 0x1F, 0x1E, 0x11, 0x03, 0x5B, // 24 ~ 31 0x18~0x1F
        0x67, 0x2E, 0x2D, 0x20, 0x12, 0x05, 0x04, 0x5C, // 32 ~ 39 0x20~0x27
        ...
        0x52, 0x53, 0x50, 0x4C, 0x4D, 0x48, 0x01, 0x45, // 112~ 119 0x70~0x77
        0x57, 0x4E, 0x51, 0x4A, 0x37, 0x49, 0x46, 0x54  // 120~ 127  0x78~0x7F
    };
