...
void Send_Key(BYTE table_entry, BYTE event)
{
    ...
    switch(table_entry)
    { //Work-around for combo "key Win + Left-Shift + Right-Arrow"(Ghost key).
        case 0x82:
            if(event == BREAK_EVENT)
            {
              CLEAR_MASK(pProject4,pComboKeyWin);
            }
            else
            {
              SET_MASK(pProject4,pComboKeyWin);
            }
            break;
        case 0x88:
            if(event == BREAK_EVENT)
            {
              CLEAR_MASK(pProject4,pComboKeyLShift);
            }
            else
            {
              SET_MASK(pProject4,pComboKeyLShift);
            }
            break;
        case 0x9B:
            if(event == BREAK_EVENT)
            {
              CLEAR_MASK(pProject4,pComboKeyRArrow);
            }
            else
            {
              SET_MASK(pProject4,pComboKeyRArrow);
            }
            break;
    }
    temp = OEM_Hook_Skip_Send_Key(&temp_table_entry,event);
 if(temp != 0xFF)
    {
    ...
    if(IS_MASK_SET(SYS_MISC1, ACPI_OS)  && event==BREAK_EVENT){
      if(STICKY_MODE_GET){
        if( FnStickKey == 1 ) {
          FnStickKey = 0;
          STICKY_MODE_CLR;
          Scanner_State &= ~Scanner_State_FN;
...
void special_code(BYTE code_byte, BYTE event)
{
    ...
    static const BYTE code key126_ctrl_mk[]   = {0x14,0xE0,0x7E,0xE0,0xF0,0x7E,0xF0,0x14,0x00};
