...
void service_PS2_data(void)
{
  ...
  {
    if(Ccb42_DISAB_AUX||IS_MASK_SET(SysStatus2,LidCloseKBMSDis))
    {
            SetPS2InhibitTime(InactiveTime);
      return;
    }
...
void ProcessMouseData(BYTE channel)
{
    ...
    if(IS_MASK_SET(KBHISR,OBF)||IS_MASK_SET(KBHISR,IBF)||(Ccb42_DISAB_AUX)||IS_MASK_SET(SysStatus2,LidCloseKBMSDis))
