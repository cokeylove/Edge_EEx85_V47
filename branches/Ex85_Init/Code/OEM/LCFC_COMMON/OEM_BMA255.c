...
/*
*******************************************************************************
* Function name: ECResetBMS255
*
* Descriptoin: This is to reset BMS255 by EC command.                   
*
* Arguments:
*       none             
* Return Values:
...
void BMA255_SelfTest(void)
{
	switch(GSensorSelfTestState & 0x37)
	{
	  ...
	  case BMA255_SELF_TEST_GET_X_AXIS:
	  	if(!bRWSMBus(SMbusCh4, SMbusRW, BMA255_ADDRESS,BMA255_ACCD_X_LSB,&GSensorTmpX_1 ,SMBus_NoPEC))
	  	{
		  		GSSmbusFailCnt++;
		  }
		  else
	  ...
	  case BMA255_SELF_TEST_GET_Y_AXIS:
	  	if(!bRWSMBus(SMbusCh4, SMbusRW, BMA255_ADDRESS,BMA255_ACCD_Y_LSB,&GSensorTmpY_1 ,SMBus_NoPEC))
	  	{
		  		GSSmbusFailCnt++;
		  }
		  else
