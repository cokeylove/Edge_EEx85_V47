...
static void Mp_Set_Sn_Dummy(BYTE *sn_char) {
    BYTE i;
...
static BOOL Mp_Get_SerialNumber(BYTE *sn_char) {
    BOOL rc;
...
*******************************************************************************
*/
static BOOL Mp_Check_Milepost(BYTE milepost, BYTE *err_code) {
    BOOL rc;
    rc = TRUE;
...
/*
*******************************************************************************
* Function name: ErrorCodeBeepProcess           
*
* Descriptoin: 
*
* Arguments:
...
{
	if((SystemIsS0 || (SysPowState==SYSTEM_S5_S0) && MuteCount == 0))
	{
		MpHousekeeper(0x01);
...
{
   //CLEAR_MASK(CCG3Status,TypeCFwUpdating);
}
