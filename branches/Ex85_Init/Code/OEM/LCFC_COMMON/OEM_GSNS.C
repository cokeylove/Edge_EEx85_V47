...
void GSensorDeviceID(void) {
  ...
  if ( IS_MASK_CLEAR(GS.Status,GS_Enable) ) { // Scan G-sensor raw data if is enabled.  
	...
	if(GSSmbusFailCnt)
	{
		if (!bRWSMBus(SMbusCh4, SMbusRB, LIS3DH_ADDRESS, Who_AM_I, &GSDeviceID, 0)) {
			GSSmbusFailCnt++;
		}
		else
		{
	...
	if(GSSmbusFailCnt)
	{
		if (!bRWSMBus(SMbusCh4, SMbusRB, LIS3DSH_ADDRESS, Who_AM_I, &GSDeviceID, 0)){
		    GSSmbusFailCnt++;
    	}
		else
		{
	...
	if(GSSmbusFailCnt)
	{		  
		if (!bRWSMBus(SMbusCh4, SMbusRB, KX022_ADDRESS, Who_AM_I, &GSDeviceID, 0)){
      		GSSmbusFailCnt++;
    	}
		else
		{
	...
	if(GSSmbusFailCnt)
	{
		if (!bRWSMBus(SMbusCh4, SMbusRB, KX023_ADDRESS, Who_AM_I, &GSDeviceID, 0)){
        	GSSmbusFailCnt++;
      	}
		else
		{
	...
	if(GSSmbusFailCnt)
	{
	  if (!bRWSMBus(SMbusCh4, SMbusRB, BMA255_ADDRESS, BMA255_BGW_CHIPID, &GSDeviceID, 0)){
      GSSmbusFailCnt++;
    }
		else
		{
...
//		GSENSOR_OUT_Y_H = KX023_YOUT_H;
  ...
  if (!bRWSMBus(SMbusCh4, SMbusWB, GSENSOR_ADDRESS, GSENSOR_CTRL_REGa, &SmbBuff, 0)) {
    GSSmbusFailCnt++;
    return;
  }
	else
	...
	if (!bRWSMBus(SMbusCh4, SMbusWB, GSENSOR_ADDRESS, GSENSOR_CTRL_REGb, &SmbBuff2, 0)) {
		GSSmbusFailCnt++;
		return;
	}
	else
...
//  BYTE  *TmpValH = (BYTE *)(&TmpVal);
  if ( IS_MASK_CLEAR(GS.Status,GS_Enable)
	  	
#if Support_GSNS_VS_PWR	
	|| !Read_SUSP() || (StableDelay > 0)
  ...
  //
		I2C_DATA = GSENSOR_OUT_X_L;
		if (!I2C_Write_to_Read(SMbusCh4, GSENSOR_ADDRESS, 0x01,&I2C_DATA, 0x04,&TempVal[0], 0))
		{
    	GSSmbusFailCnt++;
		}
		else
...
//  bRWSMBus(SMbusCh4, SMbusRB, GSENSOR_ADDRESS, GSENSOR_OUT_X_L, TmpValL, 0);
//  bRWSMBus(SMbusCh4, SMbusRB, GSENSOR_ADDRESS, GSENSOR_OUT_X_H, TmpValH, 0);
...
//  bRWSMBus(SMbusCh4, SMbusRB, GSENSOR_ADDRESS, GSENSOR_OUT_Y_L, TmpValL, 0);
//  bRWSMBus(SMbusCh4, SMbusRB, GSENSOR_ADDRESS, GSENSOR_OUT_Y_H, TmpValH, 0);
...
void APS_Cmd_18(BYTE nPort)  // ST pin control command
{
  ...
  GS_D0 = 0x18;
	...
	case LIS3DH_WHO_AM_I:
		...
		if(!bRWSMBus(SMbusCh4, SMbusWB, LIS3DH_ADDRESS, LIS3DH_CTRL_REG4, &SmbBuff, 0)){
			GSSmbusFailCnt++;
		}
		else
		{
	...
	case LIS3DSH_WHO_AM_I:
		...
		if(!bRWSMBus(SMbusCh4, SMbusWB, LIS3DSH_ADDRESS, LIS3DSH_CTRL_REG5, &SmbBuff, 0)){
			GSSmbusFailCnt++;
		}
		else
		{
	...
	case KX022_WHO_AM_I:
		...
		if(!bRWSMBus(SMbusCh4, SMbusWB, KX022_ADDRESS, KX023_SELF_TEST, &SmbBuff, 0)){
			GSSmbusFailCnt++;
		}
		else
		{
	...
	case KX023_WHO_AM_I:
		...
		if(!bRWSMBus(SMbusCh4, SMbusWB, KX023_ADDRESS, KX023_SELF_TEST, &SmbBuff, 0)){
			GSSmbusFailCnt++;
		}
		else
		{
  	...
  	case BMA255_BGW_CHIPID_VALUE:
    	...
    	else
    	{
			...
			if(!bRWSMBus(SMbusCh4, SMbusWB, BMA255_ADDRESS, BMA255_PMU_SELF_TEST, &SmbBuff, 0)){
				GSSmbusFailCnt++;
			}else{
	    	GSSmbusFailCnt=0;
			}
	    ...
	    if(!bRWSMBus(SMbusCh4, SMbusWB, BMA255_ADDRESS, BMA255_PMU_RANGE, &SmbBuff, 0)){
				GSSmbusFailCnt++;
			}else{
	    	GSSmbusFailCnt=0;
			}
