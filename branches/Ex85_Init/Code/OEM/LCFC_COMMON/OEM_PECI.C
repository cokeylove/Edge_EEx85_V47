...
void PECI_InitTimeOutTimer(void)
{
    ...
    TH1 = Timer_2ms>>8;        // Set timer1 counter 30ms
    TL1 = Timer_2ms;           // Set timer1 counter 30ms
...
#if Support_CPUCOMMFAIL_shutdown	
...
#endif	//Support_CPUCOMMFAIL_shutdown
    {
      if ( PECI_Err_CNT > 68 )    // 30 Sec.
      {
        ...
        RSMRST_shutdown();
#else		
				SET_MASK(pProject3,pUpdateCpuTempFromBios);
     		ECSMI_SCIEvent(CPUTemp_FromBIOS); //Notice BIOS to update CPU temp
#endif	//Support_CPUCOMMFAIL_shutdown		
        #endif  // EN_PwrSeqTest
      }
    }
    ...
    if( PECI_CMD(PECI_CPU_ADDR, 0x01, 0x02, PECI_CMD_GetTemp, &PECIBuffer,&PECIBuffer) == 0x00 ) // Read OK
    {
      ...
      PECI_GET_TEMPH = *(&PECIBuffer+0x01);
        ...
        {                    // not reliable at temperatures above Tjmax
            u4CpuThermal = TJMAX - tempoffset;    // Save temperature
        }
        else
        {  // Over Tjmax.
          u4CpuThermal = TJMAX;          // The same as TjMax.
        }
    PECI_Err_CNT = 0;  // Clear error count.
    CLEAR_MASK(ERR_THMSTS, b3PECITempEr);    // Clear bit3 CPU read temperature error.
