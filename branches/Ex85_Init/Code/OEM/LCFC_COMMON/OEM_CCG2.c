...
void InitialCCG2(void) {
	...
	if(Read_TYPEC_INT())
	{
  	...
  	{
    	//Get Event number
			if(!bRWSMBus(SMbusChA, SMbusRW, TypeC_Addr, RESPONSE_REGISTER, &WSMbusTemp01, SMBus_NoPEC))
			{
				TypeCCommFailCnt++;
			}
			else
	      ...
	      if(TypeCEventNum == 0x80)
	      {
	      	//To de-assert INTR pin to high.
					WSMbusTemp01 = 0x01;
	  			if(!bRWSMBus(SMbusChA, SMbusWB, TypeC_Addr, INTR_REG, &WSMbusTemp01, SMBus_NoPEC))
	  			{
	  				TypeCCommFailCnt++;
	  			}
	  			else
