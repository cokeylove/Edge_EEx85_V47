...
#if Support_TYPE_C
  SendPowerModeChangeToPdc();//ZHUANGQI
 #endif 
}
...
void OEM_SYSTEM_S0_S5 (void){
  ...
  ECSleepCount = pWorkAround+2;        // Delay entry EC sleep.
  FanAutomaticmodeEn;
  FanMaximumspeedCLR;
  u4ManualFanPRM = 0;  
  CLEAR_MASK(uReserve07,b7Fan2Manual);
...
#if KB_FnSticky_Key   
  if ( IS_MASK_CLEAR(CURRENT_STATUS, ENTER_S3)){  //if S3 don't clean sticky status
    STICKY_MODE_CLR;
    STICKY_LOCK_CLR;
    Scanner_State &= ~Scanner_State_FN;
    temp_scanner_state.field.tFN = 0; 
