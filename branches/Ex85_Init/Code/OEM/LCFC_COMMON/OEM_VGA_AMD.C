...
void VGA_Initial_Temp(void)
{
  ...
  MaxCPU_MCHTemp2 = 0x00;
  MaxCPU_MCHTemp3 = 0x8E;

  if( !bWSMBusBlock(SMbusCh4, SMbusWBK, DGPU_Addr, 0x01, &MaxCPU_MCHTemp0, 4, SMBus_NoPEC)) //Load GPU register offset to SMB_ADDR
  {
    VGAErrCnt++;
  ...
  {  
  	MaxCPU_MCHTemp0 = 0xC0;
  	MaxCPU_MCHTemp1 = 0x30;
  	MaxCPU_MCHTemp2 = 0x00;
  	MaxCPU_MCHTemp3 = 0x14;

  if( !bWSMBusBlock(SMbusCh4, SMbusWBK, DGPU_Addr, 0x02, &MaxCPU_MCHTemp0, 4, SMBus_NoPEC)) // Send the data to be written into the GPU register
  {
    VGAErrCnt++;
    if( IS_MASK_CLEAR(ERR_THMSTS, b1VGATempEr) )// Check bit1 is Error.
    {
...
void VGA_Temperature(void)
{
  ...
  if(!bRSMBusBlock(SMbusCh4, SMbusRBK, DGPU_Addr, 0x03, &SMB3_DATA))
  {
    CLEAR_MASK(SysStatus2,b4VGATemp_Init); // Clear AMD dGPU init ok flag.
    VGAErrCnt++;
    if( IS_MASK_CLEAR(ERR_THMSTS, b1VGATempEr) )// Check bit1 is Error.
