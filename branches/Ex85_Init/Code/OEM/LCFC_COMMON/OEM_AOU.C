  ...
  )
{
  ...
  if(Read_AOU_DET()) {
    ...
    else if ((SysPowState != SYSTEM_S0_S3) && (SysPowState != SYSTEM_S3) && (AOU_IFG_Debounce_Cnt != 0))
    {
...
//for S3 no wake device and aou disable
  if (SystemIsS3 && !USB_PWRGET) // AOU feature disabled (should be SDP)
  { 
    if(!TurnOnUSBGET)    //no wake device
    {
      AouUsbPowerMode = NonAouUsbPowerMode = PwrOff;
    }      
  }
