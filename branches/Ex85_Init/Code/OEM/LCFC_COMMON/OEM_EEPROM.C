...
void WRITE_DATA2EEPROM(PORT_BYTE_PNTR ram_pntr, XWORD size, BYTE ROMAddress, XWORD offset)
{
  BYTE Temp8;
  PORT_BYTE_PNTR Temp_ram_pntr = ram_pntr;
  XWORD Temp_size = size;
  XWORD Temp_offest = offset;
...
void UPDATA_EEPROM_DATA(PORT_BYTE_PNTR ram_pntr, XWORD size, BYTE ROMAddress, XWORD offset)
{
   BYTE i, B256cnt, Temp8;
   XWORD TempADD, Updateadd;
...
void Update_16ByteEEPROM(void)
{
  ...
  if (pLastSID == BATTOVERTEMP_ID) {
    ...
    Write_Eflash_1Byte(EEPROMA2,(EEPROMA1_16Byte+i),(j*16)+0x0A,BatteryAlarm);        
  }
