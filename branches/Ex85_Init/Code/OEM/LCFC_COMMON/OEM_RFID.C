...
void writeRFIDProtect(BYTE BANKx,BYTE acc)
{
  ...
  BYTE rMark;
	...
	if( BANKx == 0x06 )
	{
		nRTempL = (RFID_EEPROM_RN4);
		nWTempL = (RFID_EEPROM_WN4);
		nRTempH = (RFID_EEPROM_RN4>>8);
		nWTempH = (RFID_EEPROM_WN4>>8);		
		nCRTempL = (RFID_EEPROM_RN0);
		nCWTempL = (RFID_EEPROM_WN0);
		nCRTempH = (RFID_EEPROM_RN0>>8);
		nCWTempH = (RFID_EEPROM_WN0>>8);		
		nTRTempL = (RFID_EEPROM_RN6);
		nTWTempL = (RFID_EEPROM_WN6);
		nTRTempH = (RFID_EEPROM_RN6>>8);
		nTWTempH = (RFID_EEPROM_WN6>>8);

		dRTempH = (dRFID_EEPROM_RN4>>8);
		dWTempH = (dRFID_EEPROM_WN4>>8);
		dCRTempH = (dRFID_EEPROM_RN0>>8);
		dCWTempH = (dRFID_EEPROM_WN0>>8);
		dTRTempH = (dRFID_EEPROM_RN6>>8);
		dTWTempH = (dRFID_EEPROM_WN6>>8);
	}
	else if( BANKx == 0x07 )
	{
		nRTempL = (RFID_EEPROM_RN6);
		nWTempL = (RFID_EEPROM_WN6);
		nRTempH = (RFID_EEPROM_RN6>>8);
		nWTempH = (RFID_EEPROM_WN6>>8);
		nCRTempL = (RFID_EEPROM_RN0);
		nCWTempL = (RFID_EEPROM_WN0);
		nCRTempH = (RFID_EEPROM_RN0>>8);
		nCWTempH = (RFID_EEPROM_WN0>>8);
		nTRTempL = (RFID_EEPROM_RN4);
		nTWTempL = (RFID_EEPROM_WN4);
		nTRTempH = (RFID_EEPROM_RN4>>8);
		nTWTempH = (RFID_EEPROM_WN4>>8);

		dRTempH = (dRFID_EEPROM_RN6>>8);
		dWTempH = (dRFID_EEPROM_WN6>>8);
		dCRTempH = (dRFID_EEPROM_RN0>>8);
		dCWTempH = (dRFID_EEPROM_WN0>>8);
		dTRTempH = (dRFID_EEPROM_RN4>>8);
		dTWTempH = (dRFID_EEPROM_WN4>>8);
	}
	else
	{
		nRTempL = (RFID_EEPROM_RN0);
		nWTempL = (RFID_EEPROM_WN0);
		nRTempH = (RFID_EEPROM_RN0>>8);
		nWTempH = (RFID_EEPROM_WN0>>8);
		nCRTempL = (RFID_EEPROM_RN4);
		nCWTempL = (RFID_EEPROM_WN4);
		nCRTempH = (RFID_EEPROM_RN4>>8);
		nCWTempH = (RFID_EEPROM_WN4>>8);
		nTRTempL = (RFID_EEPROM_RN6);
		nTWTempL = (RFID_EEPROM_WN6);
		nTRTempH = (RFID_EEPROM_RN6>>8);
		nTWTempH = (RFID_EEPROM_WN6>>8);

		dRTempH = (dRFID_EEPROM_RN0>>8);
		dWTempH = (dRFID_EEPROM_WN0>>8);
		dCRTempH = (dRFID_EEPROM_RN4>>8);
		dCWTempH = (dRFID_EEPROM_WN4>>8);
		dTRTempH = (dRFID_EEPROM_RN6>>8);
		dTWTempH = (dRFID_EEPROM_WN6>>8);
	}
